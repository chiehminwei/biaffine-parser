...
class Vocab(object):
    ...
    def numericalize(self, corpus, save_name=None):
        ...
        for words, arcs, rels, tags in zip(corpus.words, corpus.heads, corpus.rels, corpus.tags):
            ...
            # BERT9-12
            layers = []
            bert_output, _ = self.bert(torch.tensor([sentence_token_ids]).to(torch.device('cuda')), token_type_ids=torch.LongTensor([0 for i in sentence_token_ids]).to(torch.device('cuda')), attention_mask=torch.ByteTensor([1 for i in sentence_token_ids]).to(torch.device('cuda')))
            del _
